...
"""
        ...
        self._current_worker = AIWorker(endpoint, api_key, model, sys_prompt, text)
        self._current_worker.finished.connect(self._on_review_finished)
        self._current_worker.error.connect(self._on_review_error)
        self._current_worker.start()

    def _on_review_finished(self, res):
...
"""
        ...
        worker.finished.connect(lambda res: self._on_transform_done(res.get("content", "")))
        worker.error.connect(lambda err: self._on_transform_done(f"[Error: {err}]"))
        worker.start()

    def _on_transform_done(self, content):
        self.ai_status_changed.emit("ready")
